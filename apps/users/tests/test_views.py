...
from apps.user.api.serializers import driver
...
class ViewsTest(APITestCase):
    ...
    def test_users_list_with_user(self):
        ...
        response = self.client.get(reverse("users:api:users-list"))
    ...
    def test_users_list_with_superuser(self):
        ...
        response = self.client.get(reverse("users:api:users-list") + "?ordering=id")
        users = get_user_model().objects.all()
        serializer = driver.UsersListSerializer(users, many=True)
    ...
    def test_users_detail(self):
        ...
        path = reverse("users:api:users-detail", kwargs={"pk": self.user.pk})
        response = self.client.get(path=path)

        user = get_user_model().objects.get(pk=self.user.pk)
        serializer = driver.UserDetailUpdateDeleteSerializer(user)

        self.assertEquals(response.status_code, status.HTTP_200_OK)
        self.assertEquals(response.data, serializer.data)

        path = reverse("users:api:users-detail", kwargs={"pk": "0"})
        response = self.client.get(path=path)
    ...
    def test_get_profile(self):
        ...
        response = self.client.get(path)
        serializer = driver.UserProfileSerializer(self.user)
    ...
    def test_login_with_two_step_password(self):
        ...
        path = reverse("users:api:verify-two-step-password")
